...
`npm run package:github-upload` produces an upload edition whose individual files and ZIP parts remain below GitHub's 25 MB browser-upload limit. Oversized LaMa, FFmpeg and ONNX assets are reconstructed before `npm ci` and validated against pinned size and SHA-256 values. The GitHub release workflow is therefore the authoritative path to complete the missing real Chromium, Firefox, WebKit, offline/PWA and browser 4K gates without weakening the release rule.
