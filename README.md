...
- neural reconstruction uses a checksum-pinned LaMa 512 INT8 ONNX model through local ONNX Runtime Web; WebGPU is attempted only after real adapter/device initialization and WASM is the fallback; automatic tracking and temporal alignment use local template/multi-block motion and are not mislabeled as dense optical flow
