...
MODEL_URL="https://huggingface.co/g-ronimo/lama/resolve/main/lama_int8.onnx?download=true"
MODEL_SHA256="1941214c210399eb815eb2d32570ba91d5e6c4ac3de4c939bd3fb09300454972"
MODEL_SIZE="61512617"
ORT_VERSION="1.29.0"
...
model_is_valid=0
if [[ -f public/models/lama-dynamic-int8.onnx ]] \
  && [[ "$(stat -c '%s' public/models/lama-dynamic-int8.onnx)" == "${MODEL_SIZE}" ]] \
  && echo "${MODEL_SHA256}  public/models/lama-dynamic-int8.onnx" | sha256sum --check --strict >/dev/null 2>&1; then
  model_is_valid=1
fi
...
if [[ "${model_is_valid}" == "1" ]]; then
  ...
  curl --fail --location --retry 2 --connect-timeout 30 --max-time 300 \
    --output public/models/lama-dynamic-int8.onnx "${MODEL_URL}"
fi
echo "${MODEL_SHA256}  public/models/lama-dynamic-int8.onnx" | sha256sum --check --strict
[[ "$(stat -c '%s' public/models/lama-dynamic-int8.onnx)" == "${MODEL_SIZE}" ]] || { echo "AI model size mismatch" >&2; exit 65; }
rm -rf .tmp-ai-pack
